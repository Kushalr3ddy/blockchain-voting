...
//configuring ethers
const contractAddress = "0xf21cdcfd9CA7605132FCaaC3Eb85E482b738AC0e";
const contractABI = [
    {
      "inputs": [],
...
let contract;
let signer;
const provider = new ethers.providers.Web3Provider(window.ethereum,80001);//80001 from chainlink mumbai testnet
...
const getAllCandidates = async function(){
    ...
    let candidates = await contract.retrieveVotes();//call retrieve votes form contract
    for(let i =0;i<candidates.length;i++){
        let candidate = document.createElement("tr");
        //get candidate name from the candidate struct in contract
...
const getResult = async function(){
    ...
    if(document.getElementById("resultBoard")){//remove duplicate tables
    ...
    for(let i =0;i<candidates.length;i++){
        let candidate = document.createElement("tr");
        //get candidate id,name,no of votes from the candidate struct in contract
        candidate.innerHTML = `<td>${parseInt(candidates[i][0])}</td>
                            <td>${candidates[i][1]}</td>
...
const refreshPage = function(){
    setInterval(async()=>{
        ...
        if(time >0){
            ...
            voteForm.display='flex';
            showResultContainer.style.display = 'none';
        }
        else{
            timerMessage.textContent = "either no election yet or election ended";
...
const getAccount = async function(){
    ...
    if (owner == signer._address){ // check if the contract owner and metamask user addr is same and display the admin panel
...
addTheCandidate.addEventListener('click',startElection);
